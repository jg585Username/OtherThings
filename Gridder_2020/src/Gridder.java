...
public class Gridder extends javax.swing.JFrame 
    ...
    public void mouseDragged(MouseEvent e) { 
      ...
      int row = (y-offSet )/squareSize;
      int col = (x-offSet )/squareSize;
      System.out.println("Drag:  " + row + ", " + col);
    ...
    public void clickGrid( int row, int col ){
        if (jMode==false)
           colorSquare(row, col, penColor); 
        else {
           flood(row,col);
           draw();
        }
        
        System.out.println(liveNeighbors(row,col) );
    }
    
    public void flood(int row, int col) {
        if (grid[row][col]==0) {  //if black, turn w
            grid[row][col]=1;
            draw();
            try {  
                Thread.sleep(200);
    ...
    private void initComponents() {
        ...
        jPanel1Layout.linkSize(javax.swing.SwingConstants.HORIZONTAL, new java.awt.Component[] {button1, button10, button11, button12, button2, button3, button4, button5, button6, button7, button8, button9});
    ...
    public void step() {
        //sample code to flip one locatio in grid, take this out!
    ...
    private void button1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button1ActionPerformed
        int rrRow = (int) (Math.random()*grid.length);
        int rrColumn = (int) (Math.random()*grid.length);
        for (int i = 0; i < 100; i++){
            grid[rrRow][rrColumn] = 1;
        }        
        draw();
